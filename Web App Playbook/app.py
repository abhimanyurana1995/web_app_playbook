...
app = Flask(__name__)
app.secret_key = os.environ.get("SECRET_KEY", "dev-playbook-key-change-in-prod")
...
def inject_globals():
    return {"total_topics": TOTAL_TOPICS}
...
def toggle_journey_step(step_id):
    ...
    db = get_db()
    if has_notes:
        # explicit notes update — allow clearing with null/empty
        db.execute("""
        ...
        # toggle only — preserve existing notes
        db.execute("""
            INSERT INTO journey_progress (step_id, step_order, completed, completed_at, notes)
            VALUES (?, ?, ?, CASE WHEN ? = 1 THEN datetime('now') ELSE NULL END, NULL)
            ON CONFLICT(step_id) DO UPDATE SET
                completed    = excluded.completed,
                completed_at = excluded.completed_at,
                notes        = journey_progress.notes
        """, (step_id, order, completed, completed))
    db.commit()
...
def chat_stream():
    ...
    topic_id = data.get("topic_id", "").strip() or None
    history  = data.get("history") or []
    if not message:
        return jsonify({"error": "message required"}), 400

    def generate():
        for chunk in ollama_client.chat_stream(message, topic_id, history):
